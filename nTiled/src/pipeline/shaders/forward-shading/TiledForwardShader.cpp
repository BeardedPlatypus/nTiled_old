...
ForwardTiledLightShader::ForwardTiledLightShader(ShaderId shader_id,
	                                             std::string path_vertex_shader,
	                                             std::string path_fragment_shader,
	                                             glm::uvec2 tile_size,
	                                             nTiled_state::State& state) : 
	BasicForwardLightShader(shader_id, 
		                    path_vertex_shader, 
		                    path_fragment_shader),
	projector(BoxProjector()),
    light_manager(TiledLightManager(state, tile_size.x, tile_size.y, projector)) {}

void ForwardTiledLightShader::init(Camera& camera) {
	// Call Parent class init
	// ------------------------------------------------------------------------
	BasicForwardLightShader::init(camera);
...
void ForwardTiledLightShader::render(Camera& camera) {
	// start program
	glUseProgram(this->shader_program);
...
void ForwardTiledLightShader::loadShaders() {
	// Vertex Shader
	// -----------------------------------------------------------------
