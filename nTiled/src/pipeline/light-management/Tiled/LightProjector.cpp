...
bool BoxProjector::computeProjection(
  ...
  if (computeNDCProjection(light, camera, ndc_coordinates)) {
    ...
    projection.x = floor(((ndc_coordinates.x + 1.0f) * 0.5f) * viewport.x - 1) / tilesize.x;
    projection.y = floor(((ndc_coordinates.y + 1.0f) * 0.5f) * viewport.y - 1) / tilesize.y;
    projection.z = floor(((ndc_coordinates.z + 1.0f) * 0.5f) * viewport.x - 1) / tilesize.x;
    projection.w = floor(((ndc_coordinates.w + 1.0f) * 0.5f) * viewport.y - 1) / tilesize.y;
